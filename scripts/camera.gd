...
func _toggle_binocular() -> void:
	...
	if is_binocular:
		...
		previous_camera_rotation = previous_camera.rotation_degrees
		base_rotation            = previous_camera.rotation_degrees
		_apply_rotation()
		make_current()
	else:
		pitch = 0.0
		...
		if previous_camera:
			...
			push_warning("_toggle_binocular: previous_camera is null, cannot restore.")
