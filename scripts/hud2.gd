...
func _ready() -> void:
	...
	# Locate the telegraph wherever it lives in your scene
...
func _build_top_bar() -> void:
	...
	# TRANSPARENT background — no blue overlay
	bar.add_theme_stylebox_override("panel", _make_panel_style(
		Color(0.0, 0.0, 0.0, 0.0), Color(0.12, 0.35, 0.63, 0.0), 0, 0, 0, 0))
	root.add_child(bar)
...
func _build_left_panel() -> void:
	...
	# Radar container placeholder (the existing Radar node goes here)
...
func _build_center_panel() -> void:
	...
	lbl_p.add_theme_color_override("font_color", Color(1.0, 0.3, 0.3, 0.5))
	...
	lbl_s.add_theme_color_override("font_color", Color(0.2, 1.0, 0.5, 0.5))
...
func _build_right_panel() -> void:
	...
	# Telegraph notch list — built procedurally
	var notches := [
...
func _update_turn_indicators(steering: float) -> void:
	...
	pa.arrow_color = Color(1.0, 0.2, 0.2) if steering < -DEAD else Color(0.5, 0.1, 0.1)
	sa.arrow_color = Color(0.3, 1.0, 0.4) if steering >  DEAD else Color(0.1, 0.35, 0.1)
...
func _process(delta: float) -> void:
	...
	var rot_clamped := clampf(disp_rot / ROT_CLAMP, -1.0, 1.0)
...
class CompassRose extends Control:
	...
	func _draw() -> void:
		...
		# COG line (heading offset slightly to simulate leeway)
...
class ROTBar extends Control:
	...
	func _draw() -> void:
		...
		var col    := Color(0.3, 1.0, 0.5) if rot_normalized > 0 else Color(1.0, 0.35, 0.35)
